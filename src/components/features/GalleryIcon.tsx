...
const GalleryIcon: React.FC<GalleryIconProps> = ({ width = 64, height = 64, className = "" }) => {
  ...
  // Handle mouse movement to move the eye
  useEffect(() => {
    if (!whimsyMode || !svgRef.current || !eyeRef.current) return;

    const handleMouseMove = (e: MouseEvent) => {
      if (!svgRef.current || !eyeRef.current) return;
      ...
      // Limit eye movement within a reasonable range (15-17 for X, 7-9 for Y)
      const eyeX = Math.max(15, Math.min(17, normalizedX));
      const eyeY = Math.max(7, Math.min(9, normalizedY));

      // Update eye position
      setPosition({ x: eyeX, y: eyeY });
    };

    // Add mouse move listener
    document.addEventListener("mousemove", handleMouseMove);

    // Set up blinking interval (every 3 seconds)
    blinkIntervalRef.current = setInterval(() => {
      setIsBlinking(true);
      setTimeout(() => setIsBlinking(false), 150); // Blink for 150ms
    }, 3000);
  ...
  // Calculate the eye path with current position and blink state
  const getEyePath = () => {
    const { x, y } = position;

    if (isBlinking && whimsyMode) {
      // Flattened eye when blinking
      return `M${x - 2} ${y}C${x - 2} ${y + 0.2} ${x + 2} ${y + 0.2} ${
        x + 2
      } ${y}C${x + 2} ${y - 0.2} ${x - 2} ${y - 0.2} ${x - 2} ${y}Z`;
    }

    // Normal eye (circle)
    return `M${x + 2} ${y}C${x + 2} ${y + 1.10457} ${x + 1.1046} ${
      y + 2
    } ${x} ${y + 2}C${x - 1.1046} ${y + 2} ${x - 2} ${y + 1.10457} ${
      x - 2
    } ${y}C${x - 2} ${y - 1.10457} ${x - 1.1046} ${y - 2} ${x} ${y - 2}C${
      x + 1.1046
    } ${y - 2} ${x + 2} ${y - 1.10457} ${x + 2} ${y}Z`;
  };
  ...
  return (
    <svg
      ...
      className={`${className} ${whimsyMode ? "rotate-12" : ""}`}
    >
      {/* Base gallery icon paths */}
      <path
        fillRule="evenodd"
        clipRule="evenodd"
      ...
      <path
        ...
        fill="#76a6c9"
        style={{
          transition: whimsyMode ? "none" : "all 0.3s ease-out",
        }}
      />
    </svg>
  );
