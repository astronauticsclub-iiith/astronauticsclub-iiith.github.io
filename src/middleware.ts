...
export default withAuth(
  function middleware(req) {
    ...
    const isBlogAuthorRoute = req.nextUrl.pathname.startsWith('/clicktiy-clackity-blogs-are-my-property');
    ...
    if (token) {
      ...
      if (isBlogAuthorRoute && !userRoles.some(role => ['admin', 'writer'].includes(role))) {
        return NextResponse.redirect(new URL('/stay-away-snooper', req.url));
      }
      ...
      if (isLoginRoute) {
        if (userRoles.includes('admin')) {
          ...
          return NextResponse.redirect(new URL('/clicktiy-clackity-blogs-are-my-property', req.url));
        }
      }
    }
...
export const config = {
  matcher: [
    '/imtheboss/:path*',
    '/clicktiy-clackity-blogs-are-my-property/:path*',
    '/let-me-innn/:path*',
    '/api/upload/:path*',
    '/api/users/:path*',
    '/api/my-blogs/:path*',
